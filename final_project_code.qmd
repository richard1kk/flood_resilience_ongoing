---
title: "Environmental Equity, Health Vulnerability, and Topographic Flood Susceptibility in Washington, D.C.:A Spatial Analysis of Risk and Resilience"
author: "Richard Luo"
format: html
editor: visual
---
...
diabetes_plot <- ggplot(census_diabetes) +
  geom_sf(aes(fill = Diabetes,
              ...
              "<br>Diabetes Rate: ", round(Diabetes, 1), "%")), 
          color = "white", size = 0.1, alpha = 0.85) +
  scale_fill_viridis(
    option = "D",
    na.value = "gray90",
...
p <- ggplot(flood_map) +
  geom_sf(aes(fill = percent_flood_500,
          ...
          color = NA) +
  scale_fill_viridis_c(option = "mako", name = "500-Year Flood %", direction = -1) +
...
# 15. % of Buildings within Threshold Radius

# 16. % of Census Tract Within Threshold Radius
...
### Percentage of Area within a Tract 407 meters or less from Water (By Pixelated Area)

```{r}

# Load tracts
water_dc_tracts <- tracts(state = "DC", year = 2020, class = "sf") %>%
  st_transform(32618)  # UTM Zone 18N — adjust if needed

# Load waterbodies
dc_water <- read_sf("Waterbodies.shp") %>%
  st_transform(st_crs(water_dc_tracts)) %>%
  st_make_valid()

# Create a raster template (30m resolution, or adjust)
template_raster <- rast(ext(water_dc_tracts), resolution = 30, crs = st_crs(water_dc_tracts)$wkt)

# Rasterize water polygons (water = 1, NA elsewhere)
water_raster <- rasterize(vect(dc_water), template_raster, field = 1, background = NA)

# Distance to nearest water cell (in meters)
dist_raster <- distance(water_raster)

dc_tracts_proj <- st_transform(dc_tracts, crs = crs(dist_raster))

threshold <- 407.495  # meters

# Use exactextractr to calculate % of pixels under the threshold
...
### % of Buildings Within 407 Meters
...
final_df <- df_numeric %>%
  ...
  mutate(ground_minus_base = ground_elevation - base_elevation)
